...
let err_malloc_zero_size         = "malloc: zero-sized allocation"
let err_memcpy_overlap           = "memcpy: overlapping memory regions"
let err_ptrplus_no_base_var     = "pointer arithmetic: no base pointer variable found"
let err_ptrplus_use_after_free  = "pointer arithmetic: freed memory block"
let err_ptrplus_null            = "pointer arithmetic: null pointer (Base==End==0)"
let err_ptrplus_below_base      = "pointer arithmetic: result below Base"
let err_ptrplus_above_end       = "pointer arithmetic: result above End"
let err_ptrplus_missing_base    = "pointer arithmetic: missing Base constraint"
let err_ptrplus_missing_end     = "pointer arithmetic: missing End constraint"
let err_ptrsub_no_base_var       = "pointer subtraction: no base pointer variable found"
let err_ptrsub_use_after_free    = "pointer subtraction: freed memory block"
let err_ptrsub_null              = "pointer subtraction: null pointer (Base==End==0)"
let err_ptrsub_different_blocks  = "pointer subtraction: pointers in different blocks"
let err_ptrsub_below_base        = "pointer subtraction: pointer below Base"
let err_ptrsub_above_end         = "pointer subtraction: pointer above End"
...
(* State equality is defined in AtlasDomain.ml — the domain module owns deduplication logic *)
...
let interval_trim_and_convert var_id interval_off interval_end cell_size hp =
  ...
  let cells, fresh_ids = match hp with
    ...
    | Formula.BlockPointsTo _ | Formula.UniformBlockPointsTo _ ->
      let rec chop cells ids off =
        if Int64.compare off trim_end >= 0 then (List.rev cells, List.rev ids)
          ...
          chop (cell :: cells) (dest_id :: ids) (Stdlib.Int64.add off cell_size)
      in
      chop [] [] trim_off
  in
...
let rec heap_extract_interval_local state var_id offset size cell_size cell_typ
  ...
  let state = { state with
    ...
    current = { state.current with
      spatial = curr_spatial @ curr_rest } }
  in
...
(** Extracts cells from the interval \[offset, offset + size)\] for formal
    memory (allocated by the caller, present in missing). Processes only
    [curr_in] predicates — cells created from [BlockPointsTo] splits are
    mirrored to missing (same fresh ids in both), while existing [ExpPointsTo]
